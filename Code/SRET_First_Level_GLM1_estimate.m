function SRET_First_Level_GLM_estimate(subj)

% Creates spm 1st level design matrix and estimates job 

%Script loads in basic experiment info and specifies condition
%onsets/durations and nuisance regressors: motion parameters and 1st order 
%temporal derivatives, and outlier timepoints based on framewise displacement.  
...
base_dir='/path to data/Data/'; %location of folder that contains each participant's data (e.g., S01, S02)
Regressor_dir='/path to regressors/Regressors/';
SessionList={'Run1'};
cd(base_dir)

Session='ses-pre'; %%% update manually %%%
...
%%
    ...
    cd([base_dir sub_list{n} fs Session fs]);
    mkdir('First_level_analysis_GLM1');
    stats_dir=[base_dir sub_list{n} fs Session fs 'First_level_analysis_GLM1' fs]; % directory to save output files      
    cd(stats_dir); 
    if exist('SPM.mat', 'file'); delete('SPM.mat','*.nii','*.hdr');end  % Go to output directory and delete any existing files
    
  %% Load in condition onsets and durations 
  ...
  for k = 1:length(SessionList) 
    ...
    Functional_Dir=[base_dir sub_list{n} fs Session fs SessionList{k} fs]; %directory with subject's data for the current session
    cd(Functional_Dir);
...
Contrast_Names={'NegSelf_minus_Neg Case' 'NegCase_minus_Neg Self' 'PosSelf_minus_PosCase' 'PosCase_minus_Pos Self'};
%cond order: 1) neg self, 2) pos self, 3) neg case, 4) pos case, 5) questions 
contrast_vec={[1 0 -1 0 0] [-1 0 1 0 0] [0 1 0 -1 0] [0 -1 0 1 0]}; 
      
for i=1:length(contrast_vec)
         jobs{1}.stats{1}.con.consess{i}.tcon.name=Contrast_Names{i};
           ContrastVector=[contrast_vec{i} zeros(1,7) ...  %Run 1 one conditions and 7 motion regressors 
           zeros(1,1)];                           %session constants 
