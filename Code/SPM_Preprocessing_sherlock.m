...
numRuns=1; % number of functional runs
data_dir='/path to data/Data/';
Session='ses-pre';

sub_dir=[data_dir subj fs Session fs];
cd(sub_dir)
mkdir('Jobs')
jobs_dir=[sub_dir 'Jobs/'];
...
cd(func_dir)
%func_file=gunzip([func_dir '*.nii.gz']); %unzip functional data
func=ls('*.nii');
...
cd(anat_dir)
%anat_file=gunzip([anat_dir '*.nii.gz']); %unzip anatomical data
anat=ls('*.nii'); 
...
rpdx = abs(rpdx);
fd = sum(rpdx,2);
save('Framewise_Displacement_timecourse.mat', 'fd');
end
